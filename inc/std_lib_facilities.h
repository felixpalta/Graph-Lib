...
template< class T> struct Vector : public std::vector<T> {
	...
	T& operator[]( int i) // rather than return at(i);
	{
		if (i<0||this->size()<=(unsigned)i) throw Range_error(i);
		return std::vector<T>::operator[]((unsigned)i);
	}
...
struct String : std::string {
	...
	String(unsigned int sz, char val) :std::string(sz,val) {}
	template<class Iter> String(Iter p1, Iter p2) : std::string(p1,p2) { }

	char& operator[]( int i) // rather than return at(i);
	{
		if (i<0||size()<=(unsigned)i) throw Range_error(i);
		return std::string::operator[]((unsigned)i);
	}
